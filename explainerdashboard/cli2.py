...
@click.pass_context
def explainerdashboard(explainer_filepath, no_browser):
    if not no_browser:
        click.echo("browser!")
    click.echo("running dashboard!")

@explainerdashboard.command()
def build():
    click.echo('Building explainer!')

@explainerdashboard.command()
def test():
    click.echo('Testing dashboard!')
