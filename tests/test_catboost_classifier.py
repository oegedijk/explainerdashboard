...
def test_random_index(precalculated_catboost_classifier_explainer):
    ...
    assert isinstance(
        precalculated_catboost_classifier_explainer.random_index(return_str=True), str
    )


def test_ordered_cats(precalculated_catboost_classifier_explainer):
    assert precalculated_catboost_classifier_explainer.ordered_cats("Sex") == [
        "NOT_ENCODED",
        "Sex_female",
        "Sex_male",
    ]
    assert precalculated_catboost_classifier_explainer.ordered_cats(
        "Deck", topx=2, sort="alphabet"
    ) == ["Deck_A", "Deck_B"]

    assert isinstance(
        precalculated_catboost_classifier_explainer.ordered_cats("Deck", sort="freq"),
        list,
    )
    assert isinstance(
        precalculated_catboost_classifier_explainer.ordered_cats(
            "Deck", topx=3, sort="freq"
        ),
        list,
    )
    assert isinstance(
        precalculated_catboost_classifier_explainer.ordered_cats("Deck", sort="shap"),
        list,
    )
    assert isinstance(
        precalculated_catboost_classifier_explainer.ordered_cats(
...
def test_pred_percentiles(precalculated_catboost_classifier_explainer):
    assert isinstance(
        precalculated_catboost_classifier_explainer.pred_percentiles(), np.ndarray
    )


def test_columns_ranked_by_shap(precalculated_catboost_classifier_explainer):
    assert isinstance(
        precalculated_catboost_classifier_explainer.columns_ranked_by_shap(), list
    )


def test_permutation_importances(precalculated_catboost_classifier_explainer):
    assert isinstance(
        precalculated_catboost_classifier_explainer.get_permutation_importances_df(),
        pd.DataFrame,
    )
...
def test_top_interactions(precalculated_catboost_classifier_explainer):
    assert isinstance(
        precalculated_catboost_classifier_explainer.top_shap_interactions("Age"), list
    )
    assert isinstance(
        precalculated_catboost_classifier_explainer.top_shap_interactions(
            "Age", topx=4
        ),
        list,
    )
...
def test_contrib_df(precalculated_catboost_classifier_explainer):
    assert isinstance(
        precalculated_catboost_classifier_explainer.get_contrib_df(0), pd.DataFrame
    )
    assert isinstance(
        precalculated_catboost_classifier_explainer.get_contrib_df(
            X_row=precalculated_catboost_classifier_explainer.X.iloc[[0]]
        ),
        pd.DataFrame,
    )
...
def test_shap_base_value(precalculated_catboost_classifier_explainer):
    assert isinstance(
        precalculated_catboost_classifier_explainer.shap_base_value(),
        (np.floating, float),
    )


def test_shap_values_shape(precalculated_catboost_classifier_explainer):
    assert precalculated_catboost_classifier_explainer.get_shap_values_df().shape == (
        len(precalculated_catboost_classifier_explainer),
        len(precalculated_catboost_classifier_explainer.merged_cols),
    )


def test_shap_values(precalculated_catboost_classifier_explainer):
    assert isinstance(
        precalculated_catboost_classifier_explainer.get_shap_values_df(), pd.DataFrame
    )
...
def test_pdp_df(precalculated_catboost_classifier_explainer):
    assert isinstance(
        precalculated_catboost_classifier_explainer.pdp_df("Age"), pd.DataFrame
    )
    assert isinstance(
        precalculated_catboost_classifier_explainer.pdp_df("Deck"), pd.DataFrame
    )
    assert isinstance(
        precalculated_catboost_classifier_explainer.pdp_df("Age", index=0), pd.DataFrame
    )
    assert isinstance(
        precalculated_catboost_classifier_explainer.pdp_df(
            "Age", X_row=precalculated_catboost_classifier_explainer.X.iloc[[0]]
        ),
        pd.DataFrame,
    )
...
def test_plot_contributions(precalculated_catboost_classifier_explainer):
    ...
    fig = precalculated_catboost_classifier_explainer.plot_contributions(
        X_row=precalculated_catboost_classifier_explainer.X.iloc[[0]], sort="importance"
    )
    assert isinstance(fig, go.Figure)
...
def test_plot_pdp(precalculated_catboost_classifier_explainer):
    ...
    fig = precalculated_catboost_classifier_explainer.plot_pdp(
        "Age", X_row=precalculated_catboost_classifier_explainer.X.iloc[[0]]
    )
    assert isinstance(fig, go.Figure)
...
def test_pred_probas(precalculated_catboost_classifier_explainer):
    assert isinstance(
        precalculated_catboost_classifier_explainer.pred_probas(), np.ndarray
    )
...
def test_metrics(precalculated_catboost_classifier_explainer):
    assert isinstance(precalculated_catboost_classifier_explainer.metrics(), dict)
    assert isinstance(
        precalculated_catboost_classifier_explainer.metrics(cutoff=0.9), dict
    )
    assert isinstance(
        precalculated_catboost_classifier_explainer.metrics_descriptions(cutoff=0.9),
        dict,
    )
...
def test_precision_df(precalculated_catboost_classifier_explainer):
    assert isinstance(
        precalculated_catboost_classifier_explainer.get_precision_df(), pd.DataFrame
    )
    assert isinstance(
        precalculated_catboost_classifier_explainer.get_precision_df(multiclass=True),
        pd.DataFrame,
    )
    assert isinstance(
        precalculated_catboost_classifier_explainer.get_precision_df(quantiles=4),
        pd.DataFrame,
    )
...
def test_lift_curve_df(precalculated_catboost_classifier_explainer):
    assert isinstance(
        precalculated_catboost_classifier_explainer.get_liftcurve_df(), pd.DataFrame
    )
...
def test_plot_cumulative_precision(precalculated_catboost_classifier_explainer):
    ...
    fig = precalculated_catboost_classifier_explainer.plot_cumulative_precision(
        percentile=0.1, pos_label=0
    )
    assert isinstance(fig, go.Figure)


def test_plot_confusion_matrix(precalculated_catboost_classifier_explainer):
    fig = precalculated_catboost_classifier_explainer.plot_confusion_matrix(
