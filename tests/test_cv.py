...
def test_clas_cv_permutation_importances(classifier_explainer_with_cv):
    assert isinstance(
        classifier_explainer_with_cv.permutation_importances(), pd.DataFrame
    )
    assert isinstance(
        classifier_explainer_with_cv.permutation_importances(pos_label=0), pd.DataFrame
    )
...
def test_reg_cv_permutation_importances(regression_explainer_with_cv):
    assert isinstance(
        regression_explainer_with_cv.permutation_importances(), pd.DataFrame
    )
    assert isinstance(
        regression_explainer_with_cv.permutation_importances(pos_label=0), pd.DataFrame
    )
