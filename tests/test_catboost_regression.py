...
def test_random_index(precalculated_catboost_regression_explainer):
    ...
    assert isinstance(
        precalculated_catboost_regression_explainer.random_index(return_str=True), str
    )
...
def test_pred_percentiles(precalculated_catboost_regression_explainer):
    assert isinstance(
        precalculated_catboost_regression_explainer.pred_percentiles(), np.ndarray
    )


def test_columns_ranked_by_shap(precalculated_catboost_regression_explainer):
    assert isinstance(
        precalculated_catboost_regression_explainer.columns_ranked_by_shap(), list
    )


def test_ordered_cats(precalculated_catboost_regression_explainer):
    assert precalculated_catboost_regression_explainer.ordered_cats("Sex") == [
        "NOT_ENCODED",
        "Sex_female",
        "Sex_male",
    ]
    assert precalculated_catboost_regression_explainer.ordered_cats(
        "Deck", topx=2, sort="alphabet"
    ) == ["Deck_A", "Deck_B"]

    assert isinstance(
        precalculated_catboost_regression_explainer.ordered_cats("Deck", sort="freq"),
        list,
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.ordered_cats(
            "Deck", topx=3, sort="freq"
        ),
        list,
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.ordered_cats("Deck", sort="shap"),
        list,
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.ordered_cats(
            "Deck", topx=3, sort="shap"
        ),
        list,
    )
...
def test_get_col(precalculated_catboost_regression_explainer):
    ...
    assert isinstance(
        precalculated_catboost_regression_explainer.get_col("Age"), pd.Series
    )
...
def test_permutation_importances(precalculated_catboost_regression_explainer):
    assert isinstance(
        precalculated_catboost_regression_explainer.get_permutation_importances_df(),
        pd.DataFrame,
    )
...
def test_metrics(precalculated_catboost_regression_explainer):
    ...
    assert isinstance(
        precalculated_catboost_regression_explainer.metrics_descriptions(), dict
    )
...
def test_contrib_df(precalculated_catboost_regression_explainer):
    assert isinstance(
        precalculated_catboost_regression_explainer.get_contrib_df(0), pd.DataFrame
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.get_contrib_df(0, topx=3),
        pd.DataFrame,
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.get_contrib_df(
            0, sort="importance"
        ),
        pd.DataFrame,
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.get_contrib_df(
            X_row=precalculated_catboost_regression_explainer.X.iloc[[0]]
        ),
        pd.DataFrame,
    )
...
def test_contrib_summary_df(precalculated_catboost_regression_explainer):
    assert isinstance(
        precalculated_catboost_regression_explainer.get_contrib_summary_df(0),
        pd.DataFrame,
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.get_contrib_summary_df(0, topx=3),
        pd.DataFrame,
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.get_contrib_summary_df(0, round=3),
        pd.DataFrame,
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.get_contrib_summary_df(
            0, sort="high-to-low"
        ),
        pd.DataFrame,
    )
...
def test_shap_base_value(precalculated_catboost_regression_explainer):
    assert isinstance(
        precalculated_catboost_regression_explainer.shap_base_value(),
        (np.floating, float),
    )


def test_shap_values_shape(precalculated_catboost_regression_explainer):
    assert precalculated_catboost_regression_explainer.get_shap_values_df().shape == (
        len(precalculated_catboost_regression_explainer),
        len(precalculated_catboost_regression_explainer.merged_cols),
    )


def test_shap_values(precalculated_catboost_regression_explainer):
    assert isinstance(
        precalculated_catboost_regression_explainer.get_shap_values_df(), pd.DataFrame
    )
...
def test_pdp_df(precalculated_catboost_regression_explainer):
    assert isinstance(
        precalculated_catboost_regression_explainer.pdp_df("Age"), pd.DataFrame
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.pdp_df("Sex"), pd.DataFrame
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.pdp_df("Deck"), pd.DataFrame
    )
    assert isinstance(
        precalculated_catboost_regression_explainer.pdp_df("Age", index=0), pd.DataFrame
    )
    assert isinstance(
...
def test_plot_dependence(precalculated_catboost_regression_explainer):
    ...
    fig = precalculated_catboost_regression_explainer.plot_dependence(
        "Age", highlight_index=0
    )
    assert isinstance(fig, go.Figure)

    fig = precalculated_catboost_regression_explainer.plot_dependence(
        "Sex", highlight_index=0
    )
    assert isinstance(fig, go.Figure)
...
def test_plot_contributions(precalculated_catboost_regression_explainer):
    ...
    fig = precalculated_catboost_regression_explainer.plot_contributions(
        0, sort="high-to-low"
    )
    assert isinstance(fig, go.Figure)
...
def test_plot_pdp(precalculated_catboost_regression_explainer):
    ...
    fig = precalculated_catboost_regression_explainer.plot_pdp(
        "Age", X_row=precalculated_catboost_regression_explainer.X.iloc[[0]]
    )
    assert isinstance(fig, go.Figure)
...
def test_plot_predicted_vs_actual(precalculated_catboost_regression_explainer):
    fig = precalculated_catboost_regression_explainer.plot_predicted_vs_actual(
        logs=False
    )
    assert isinstance(fig, go.Figure)
...
def test_plot_residuals(precalculated_catboost_regression_explainer):
    ...
    fig = precalculated_catboost_regression_explainer.plot_residuals(
        residuals="log-ratio", vs_actual=True
    )
    assert isinstance(fig, go.Figure)
...
def test_plot_residuals_vs_feature(precalculated_catboost_regression_explainer):
    ...
    fig = precalculated_catboost_regression_explainer.plot_residuals_vs_feature(
        "Age", residuals="log-ratio"
    )
    assert isinstance(fig, go.Figure)
...
def test_plot_y_vs_feature(precalculated_catboost_regression_explainer):
    ...
    fig = precalculated_catboost_regression_explainer.plot_y_vs_feature(
        "Age", dropna=True
    )
    assert isinstance(fig, go.Figure)
...
def test_plot_preds_vs_feature(precalculated_catboost_regression_explainer):
    ...
    fig = precalculated_catboost_regression_explainer.plot_preds_vs_feature(
        "Age", dropna=True
    )
    assert isinstance(fig, go.Figure)
