...
def test_custom_metrics(precalculated_rf_classifier_explainer):
    ...
    metrics = np.array(
        list(
            precalculated_rf_classifier_explainer.metrics(
                show_metrics=[
                    meandiff_metric1,
                    meandiff_metric2,
                    meandiff_metric3,
...
def test_pred_probas(precalculated_rf_classifier_explainer):
    ...
    assert isinstance(
        precalculated_rf_classifier_explainer.pred_probas("Survived"), np.ndarray
    )
...
def test_metrics(precalculated_rf_classifier_explainer):
    ...
    assert isinstance(
        precalculated_rf_classifier_explainer.metrics_descriptions(cutoff=0.9), dict
    )


def test_precision_df(precalculated_rf_classifier_explainer):
    assert isinstance(
        precalculated_rf_classifier_explainer.get_precision_df(), pd.DataFrame
    )
    assert isinstance(
        precalculated_rf_classifier_explainer.get_precision_df(multiclass=True),
        pd.DataFrame,
    )
    assert isinstance(
        precalculated_rf_classifier_explainer.get_precision_df(quantiles=4),
        pd.DataFrame,
    )
...
def test_lift_curve_df(precalculated_rf_classifier_explainer):
    assert isinstance(
        precalculated_rf_classifier_explainer.get_liftcurve_df(), pd.DataFrame
    )
...
def test_plot_confusion_matrix(precalculated_rf_classifier_explainer):
    fig = precalculated_rf_classifier_explainer.plot_confusion_matrix(
