...
def test_skorch_regressor_preds(skorch_regressor_explainer):
    assert isinstance(skorch_regressor_explainer.preds, np.ndarray)


def test_skorch_regressor_permutation_importances(skorch_regressor_explainer):
    assert isinstance(
        skorch_regressor_explainer.get_permutation_importances_df(), pd.DataFrame
    )


def test_skorch_regressor_shap_base_value(skorch_regressor_explainer):
    assert isinstance(
        skorch_regressor_explainer.shap_base_value(), (np.floating, float)
    )


def test_skorch_regressor_shap_values_shape(skorch_regressor_explainer):
...
def test_skorch_regressor_shap_values(skorch_regressor_explainer):
    assert isinstance(skorch_regressor_explainer.get_shap_values_df(), pd.DataFrame)


def test_skorch_regressor_mean_abs_shap(skorch_regressor_explainer):
    assert isinstance(skorch_regressor_explainer.get_mean_abs_shap_df(), pd.DataFrame)


def test_skorch_regressor_calculate_properties(skorch_regressor_explainer):
...
def test_skorch_classifier_preds(skorch_classifier_explainer):
    assert isinstance(skorch_classifier_explainer.preds, np.ndarray)


def test_skorch_classifier_pred_probas(skorch_classifier_explainer):
    assert isinstance(skorch_classifier_explainer.pred_probas(), np.ndarray)


def test_skorch_classifier_permutation_importances(skorch_classifier_explainer):
    assert isinstance(
        skorch_classifier_explainer.get_permutation_importances_df(), pd.DataFrame
    )


def test_skorch_classifier_shap_base_value(skorch_classifier_explainer):
    assert isinstance(
        skorch_classifier_explainer.shap_base_value(), (np.floating, float)
    )


def test_skorch_classifier_shap_values_shape(skorch_classifier_explainer):
...
def test_skorch_classifier_shap_values(skorch_classifier_explainer):
    assert isinstance(skorch_classifier_explainer.get_shap_values_df(), pd.DataFrame)


def test_skorch_classifier_mean_abs_shap(skorch_classifier_explainer):
    assert isinstance(skorch_classifier_explainer.get_mean_abs_shap_df(), pd.DataFrame)


def test_skorch_classifier_calculate_properties(skorch_classifier_explainer):
    skorch_classifier_explainer.calculate_properties(include_interactions=False)


def test_skorch_classifier_pdp_df(skorch_classifier_explainer):
    assert isinstance(skorch_classifier_explainer.pdp_df("col1"), pd.DataFrame)


def test_skorch_classifier_metrics(skorch_classifier_explainer):
    assert isinstance(skorch_classifier_explainer.metrics(), dict)


def test_skorch_classifier_precision_df(skorch_classifier_explainer):
    assert isinstance(skorch_classifier_explainer.get_precision_df(), pd.DataFrame)


def test_skorch_classifier_lift_curve_df(skorch_classifier_explainer):
    assert isinstance(skorch_classifier_explainer.get_liftcurve_df(), pd.DataFrame)


def test_skorch_classifier_prediction_result_df(skorch_classifier_explainer):
