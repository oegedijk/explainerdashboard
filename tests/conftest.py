...
@pytest.fixture(scope="session")
def dashboard_dumps_folder(
    tmp_path_factory, precalculated_rf_classifier_explainer, custom_dashboard
):
    ...
    custom_dashboard.to_yaml(
        dump_path / "dashboard.yaml", explainerfile=str(dump_path / "explainer.joblib")
    )
    return dump_path
...
@pytest.fixture(scope="session")
def logistic_regression_kernel_explainer(
    fitted_logistic_regression_model, classifier_data
):
    _, _, X_test, y_test = classifier_data
    explainer = ClassifierExplainer(
        fitted_logistic_regression_model,
...
@pytest.fixture(scope="session")
def linear_regression_explainer(
    fitted_linear_regression_model, regression_data, test_names
):
    _, _, X_test, y_test = regression_data
    explainer = RegressionExplainer(
...
@pytest.fixture(scope="session")
def linear_regression_kernel_explainer(
    fitted_linear_regression_model, regression_data, test_names
):
    _, _, X_test, y_test = regression_data
    explainer = RegressionExplainer(
...
@pytest.fixture(scope="session")
def rf_regression_explainer_no_y(
    fitted_rf_regression_model, regression_data, test_names
):
...
@pytest.fixture(scope="session")
def lgbm_regression_explainer(
    fitted_lgbm_regression_model, regression_data, test_names
):
    _, _, X_test, y_test = regression_data
    explainer = RegressionExplainer(
...
def catboost_classifier_explainer(classifier_data):
    ...
    model = CatBoostClassifier(iterations=5, verbose=0).fit(
        X_cats, y_cats, cat_features=[8, 9]
    )
...
def catboost_regression_explainer(regression_data):
    ...
    model = CatBoostRegressor(iterations=5, verbose=0).fit(
        X_cats, y_cats, cat_features=[8, 9]
    )
...
@pytest.fixture(scope="session")
def rf_multiclass_explainer_no_y(
    fitted_rf_multiclass_model, test_names, multiclass_data
):
...
@pytest.fixture(scope="session")
def classifier_pipeline_explainer(
    fitted_classifier_pipeline, categorical_classifier_data
):
...
@pytest.fixture(scope="session")
def classifier_pipeline_kernel_explainer(
    fitted_classifier_pipeline, categorical_classifier_data
):
    _, _, X_test, y_test = categorical_classifier_data
    return ClassifierExplainer(
        fitted_classifier_pipeline, X_test, y_test, shap="kernel"
    )
    ...
    custom_dashboard = ExplainerDashboard(
        ...
        [
            ShapDependenceComposite(
                precalculated_rf_classifier_explainer, title="Test Tab!"
            ),
            ShapDependenceComposite,
            "importances",
        ],
...
@pytest.fixture(scope="session")
def explainer_hub(
    precalculated_rf_classifier_explainer, precalculated_rf_regression_explainer
):
    hub = ExplainerHub(
        [
            ExplainerDashboard(
                precalculated_rf_classifier_explainer,
                description="Super interesting dashboard",
            ),
            ExplainerDashboard(
                precalculated_rf_regression_explainer,
                title="Dashboard Two",
                name="db2",
                logins=[["user2", "password2"]],
            ),
        ],
