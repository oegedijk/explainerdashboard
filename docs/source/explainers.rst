...
In order to start an **explainerdashboard** you first need to construct an **Explainer**.
...
- **RandomForestExplainer**: uses shap.TreeExplainer plus generates so-called shadow trees
...
**RandomForestClassifierExplainer(model, X, y)**
**RandomForestRegressionExplainer(model, X, y)**
